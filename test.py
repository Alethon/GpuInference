...
def test(
    ...
    dataloader = LoadImagesAndLabels(test_path, 'subsets\\0-19\\labels', 5, batch_size=batch_size, img_size=img_size)
    ...
    # Print mAP per class
    print('\nmAP Per Class:')
    for i, c in enumerate(load_classes(data_cfg_dict['names'])[:nC]):
        if AP_accum_count[i]:
