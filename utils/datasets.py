...
class LoadWebcam:  # for inference
    def __init__(self, img_size=416):
        ...
        self.cam.set(cv2.CAP_PROP_FRAME_WIDTH, img_size)
        self.cam.set(cv2.CAP_PROP_FRAME_HEIGHT, img_size)
        self.height = img_size
...
class LoadImagesAndLabels:  # for training
    def __init__(self, path, imagesReplacement, clsCount, batch_size=1, img_size=608, augment=False):
        with open(path, 'r') as file:
            self.img_files = file.readlines()
            self.img_files = [x.replace('\n', '') for x in self.img_files]
            self.img_files = list(filter(lambda x: len(x) > 0, self.img_files))

        self.label_files = [x.replace('images', imagesReplacement).replace('.png', '.txt').replace('.jpg', '.txt')
                            for x in self.img_files]
        self.clsCount = clsCount
        self.nF = len(self.img_files)  # number of image files
        self.nB = math.ceil(self.nF / batch_size / clsCount)  # number of batches
